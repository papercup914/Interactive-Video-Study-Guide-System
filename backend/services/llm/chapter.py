    ...
    def _build_heuristic_chapter():
        ...
        if clean_sentences and len(clean_sentences) >= 3:
            ...
            extracted_body = (
                ...
                f"첫째, 이 주제가 제시하는 주요 문제의식과 핵심 가설을 구체적으로 점검하고, 왜 이 관점이 중요한지 맥락을 짚어봅니다. "
                f"이를 통해 표면적인 현상 너머에 자리 잡은 근본적인 원인을 입체적으로 조망할 수 있습니다.\n\n"
                f"둘째, 현실에서 마주하는 다양한 변수와 도전 과제를 선제적으로 고려하여, 보다 유연하고 실질적인 해결 방안을 모색합니다."
            )
        
        return (
            f"본 챕터에서는 **{clean_title}**의 핵심 맥락과 주요 통찰을 체계적으로 살펴봅니다.\n\n"
            f"### 1. 핵심 배경 및 주요 논점\n"
            f"{clean_title}에 대한 심층적인 탐구는 이 주제가 다루는 본질적인 가치와 현실적인 시사점을 파악하는 데서 출발합니다. "
            f"단순한 지식의 나열을 넘어, 왜 이 논의가 중요한지 그 배경과 핵심 질문을 명확히 정립하는 것이 의미 있는 학습의 첫걸음입니다. "
            ...
            f"> **💡 핵심 인사이트**\n"
            f"> {clean_title}의 정수는 불확실한 환경 속에서도 흔들리지 않는 명확한 기준과 원칙을 세우는 데 있습니다. "
            ...
            f"<quiz>\n"
            f'{{"question": "{clean_title}에서 가장 중요하게 견지해야 할 기본 원칙은 무엇인가요?", '
            f'"options": ["상황의 본질을 명확히 정의하고 단계적으로 검증하기", "원인 분석 없이 단기 결과에만 집중하기", "주변 피드백을 무시하고 기존 방식만 고수하기", "모든 계획을 확정하기 전까지 실행을 무한정 미루기"], '
            f'"answer": 0, "explanation": "{clean_title}의 핵심은 본질적인 문제의식을 바탕으로 단계적인 실행과 피드백을 통해 완성해 나가는 데 있습니다."}}\n'
            f"</quiz>"
        )
