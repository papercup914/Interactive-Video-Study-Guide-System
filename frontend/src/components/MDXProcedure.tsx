...
export default function MDXProcedure(props: { children?: React.ReactNode }) {
  return (
    <InteractiveWidgetBase<ProcedureData>
      fallbackName="절차 체크리스트(Procedure) 위젯"
      render={(data) => <ProcedureUI data={data} />}
      validate={(data) => Boolean(data && Array.isArray(data.checklists) && data.checklists.length > 0)}
    >
      {props.children}
