...
export default function MDXMnemonic(props: { children?: React.ReactNode }) {
  return (
    <InteractiveWidgetBase<MnemonicData>
      fallbackName="연상 기억법(Mnemonic) 위젯"
      render={(data) => <MnemonicUI data={data} />}
      validate={(data) => Boolean(data && (data.story || (Array.isArray(data.flashcards) && data.flashcards.length > 0)))}
    >
      {props.children}
